...
internal class ListTypeAdapterFactory<E>  // Gson can instantiate it itself
private constructor() : TypeAdapterFactory {
    override fun <T> create(
        ...
        val elementTypeAdapter =
            gson.getAdapter(TypeToken.get(elementType)) as TypeAdapter<E?>
        // Note that the always-list type adapter is made null-safe, so we don't have to check nulls ourselves
        return ListTypeAdapter(elementTypeAdapter).nullSafe() as TypeAdapter<T>
    }

    private class ListTypeAdapter<E>(private val elementTypeAdapter: TypeAdapter<E>) :
        TypeAdapter<List<E>?>() {
